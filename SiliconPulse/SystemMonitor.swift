...
class SystemMonitor: ObservableObject {
    ...
    private func setupCoreCounts() {
        // Initial core setup logic remains, but we will refine it dynamically
        ...
        case 14, 16, 20, 24: // M2/M3 Ultra/Max guesses
    ...
    private func updateRealCoreUsages() {
        ...
        if !previousProcessorInfo.isEmpty && previousProcessorInfo.count == infoArray.count {
            ...
            for i in 0..<Int(numProcessors) {
                ...
                // Match with existing core definitions to keep efficiency/performance distinction
                // Note: The kernel usually orders them, but we map linearly for now
        ...
        // Store current info for next delta calculation
    ...
    private func updateMemoryUsage() {
        ...
        // precise App Memory = (internal - purgeable)
        // This usually matches Activity Monitor's "App Memory" closely
        ...
        if sysctlbyname("hw.memsize", &physicalMemory, &size, nil, 0) != 0 {
             ...
             physicalMemory = used + free + inactive + speculative
        }
    ...
    private func updateProcessInfo() {
        // Get process count using proc_listpids to avoid task port errors with sysctl
        let count = proc_listpids(1, 0, nil, 0) // 1 = PROC_ALL_PIDS
        if count > 0 {
             // proc_listpids returns bytes required, so divide by size of pid_t (Int32)
        ...
        let threadResult = sysctlbyname("hw.ncpu", &threadCount, &threadCountSize, nil, 0)
        if threadResult == 0 {
            DispatchQueue.main.async {
